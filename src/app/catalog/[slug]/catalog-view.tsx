...
function buildMessage(shopName: string, product: CatalogProduct, selected: Record<string, string>) {
  const variantParts = Object.entries(selected)
    .filter(([, v]) => v)
    .map(([k, v]) => `${k}: ${v}`)
    .join(', ');
  const variantStr = variantParts ? ` (${variantParts})` : '';
  return `Hi ${shopName}! 👋 I want to order *${product.name}*${variantStr} – ${formatCurrency(product.price)}. Can you send me the payment link? 🙏`;
}
...
function ProductCard({
  product, index, shopName, whatsappNumber, instagramHandle,
}: {
  ...
  const message = buildMessage(shopName, product, selected);
  ...
  const handleInstagram = async () => {
    ...
    if (isMobile && instagramHandle) {
      window.location.href = `instagram://user?username=${instagramHandle}`;
        ...
        {product.inStock ? (
          <div className="flex flex-col gap-2">
            ...
            {!whatsappNumber && !instagramHandle && (
              <p className="text-xs text-center text-gray-400 py-2">Contact the seller to order</p>
            )}
          </div>
        ) : (
...
export default function MerchantCatalog({
  shopName, tagline, whatsappNumber, instagramHandle, products,
    ...
    <div className="min-h-screen" style={{ background: '#faf9f7' }}>
      ...
      <div className="sticky top-0 z-10 px-4 py-4 text-white" style={{ background: 'linear-gradient(135deg, #4f46e5, #7c3aed)' }}>
        <div className="max-w-sm mx-auto flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold leading-tight">{shopName}</h1>
            {tagline && <p className="text-xs text-white/70">{tagline}</p>}
          </div>
        </div>
      </div>
        ...
        {products.length > 0 ? (
          <>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">
              {inStockCount} {inStockCount === 1 ? 'product' : 'products'} available
            </p>
      ...
      <div className="max-w-sm mx-auto px-4 py-6 text-center">
        ...
        <p className="text-[10px] text-gray-300 mt-1">Powered by D2C Agent</p>
      </div>
    </div>
  );
}
